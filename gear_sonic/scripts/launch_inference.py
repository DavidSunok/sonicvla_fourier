...
class InferenceLaunchConfig:
    ...
    deploy_zmq_out_port: int = 5557
    """ZMQ out port for the C++ deploy debug output (g1_debug topic)."""
    ...
    # Hand type
    hand_type: str = "dex3"
    """Hand type: 'dex3' (7-DOF Unitree Dex3) or 'fourier' (6-DOF Fourier FDH-6)."""
    ...
    action_log_dir: str = ""
    """Directory to log VLA tokens + actions. Empty = disabled."""
...
def main(config: InferenceLaunchConfig):
    ...
    deploy_cmd = (
        ...
        f"--zmq-out-port {config.deploy_zmq_out_port} "
    )
    ...
    inference_cmd = (
        ...
        f"--camera-host {config.camera_host} "
        f"--camera-port {config.camera_port} "
        f"--state-zmq-port {config.deploy_zmq_out_port} "
        f"--hand-type {config.hand_type}"
    )
    if config.fourier_sim_mode:
        inference_cmd += " --fourier-sim-mode"
    if config.fourier_action_scale != 1.0:
        inference_cmd += f" --fourier-action-scale {config.fourier_action_scale}"
    if config.action_log_dir:
        inference_cmd += f" --action-log-dir '{config.action_log_dir}'"
